...
//RESET DU RGBOLED
void clearScreen(short tab[(MAX_COL+1)*(MAX_ROW+1)]){
	for (char row=0; row <= MAX_ROW; row++){
		for (char col=0; col <= MAX_COL; col++){
			printPixel(row, col, 0x0000);
			tab[(int) (MAX_COL+1)*row+col] = 0;
		}
	}
}

//Affiche sur le RGBOLED le tableau
void print_tab(short *tab){
	for (char row=0; row <= MAX_ROW; row++){
		for (char col=0; col <= MAX_COL; col++){
			printPixel(row, col, tab[(MAX_COL+1)*(int)row+(int)col]);
		}
	}
}
...
void printRect(char row1, char col1, char row2, char col2, int color, int write, short *tab){
    ...
    if(col1<=col2){
        for(int i=0; i<=(col2-col1);i++){
            ...
            if (write==1){
                tab[(MAX_COL+1)*(int)row1+(int)col1+i] = color;
						}
        }
    }
        //1.2
        ...
        for(int i=0; i<=(col1-col2);i++){
            ...
            if(write==1){
                tab[(MAX_COL+1)*(int)row1+(int)col2+i] = color;
            }
        }
    }
    ...
    if(row1<=row2){
        for(int i=0; i<=(row2-row1)-1;i++){
            ...
            if(write==1){
                tab[(MAX_COL+1)*((int)row1+i+1)+(int)col1] = color;
            }
        }
    }
        //2.2
        ...
        for(int i=0; i<=(row1-row2)-1;i++){
            ...
            if(write==1){
                tab[(MAX_COL+1)*((int)row2+i)+(int)col1] = color;
            }
        }
    }
    ...
    if(row1<=row2){
        for(int i=0; i<=(row2-row1)-1;i++){
            ...
            if (write==1) {
                tab[(MAX_COL+1)*((int)row1+i+1)+(int)col2] = color;
            }
        }
    }
        //3.2 et 3.4
        ...
        for(int i=0; i<=(row1-row2)-1;i++){
            ...
            if(write==1){
                tab[(MAX_COL+1)*((int)row2+i)+(int)col2]=color;
            }
        }
    }
    //4.1 et 4.2
    if(col1<=col2){
        for(int i=0; i<=(col2-col1)-2;i++) {
            ...
            if (write==1) {
                tab[(MAX_COL+1)*(int)row2+(int)col1+i+1] = color;
            }
        }
    }
        //4.3 et 4.4
        ...
        for(int i=0; i<=(col1-col2)-2;i++) {
            ...
            if (write==1) {
                tab[(MAX_COL+1)*(int)row2+(int)col2+i+1]= color;
            }
        }
    }
}
        ...
        if(row1<=row2){
           for(int i =0; i<=(row2-row1); i++){
               for(int j =0; j<(col2-col1); j++){
                   ...
                   tab[(MAX_COL+1) * ((int)row1+i) + (int)col1 + j] = color;
               }
           }
        }
        //2
            ...
            for(int i =0; i<=(row1-row2); i++){
                for(int j =0; j<(col2-col1); j++){
                    ...
                    tab[(MAX_COL+1) * ((int)row2+i) + (int)col1 + j] = color;
                }
            }
        }
    }
        ...
        if(row1<=row2){
            for(int i =0; i<=(row2-row1); i++){
                for(int j =0; j<(col1-col2); j++){
                    ...
                    tab[(MAX_COL+1) * ((int)row1+i) + (int)col2 + j] = color;
                }
            }
        }
        //4
            ...
            for(int i =0; i<=(row1-row2); i++){
                for(int j =0; j<(col1-col2); j++){
                    ...
                    tab[(MAX_COL+1) * ((int)row2+i) + (int)col2 + j] = color;
                }
            }
        }
    }
